function plot_ipad_from_json_delsys(varargin) 
plottt0data  = 1;
if nargin < 1
...
ff = findFilesBVQX(ipadir,'*.csv'); % assumes 1 delsys *.csv exist in folder
delsysraw = convertDelsysToMat(ff{1}); % load eeg data
...
elseif exist(fullfile(ipadir,'ipad_allign_info.mat'),'file')
    ...
    % find 5hz variable names and deduce sampling rate 
    namesfind = {'dbs5hzemg','soundemg','touchemg'}; 
    sratesuse = [15/0.0135, 26/0.0135 , 26/0.0135 ];
    for nn = 1:length(namesfind)
        varnames = delsysraw.Properties.VariableNames';
        procVarnames = cellfun(@(x) lower(strrep(x,'_','')),varnames,'UniformOutput',false);
        idx5hz =  cell2mat(cellfun(@(x) any(strfind(x,namesfind{nn})),procVarnames,'UniformOutput',false));
        varidx = find(idx5hz==1);
        if length(varidx)>1
            error('can not find emg 5hz field name')
        end
        fn5hz = varnames{varidx};
        fn5hzsec = varnames{varidx-1};
        % compute sampling rate:
        for ii = 2:5e3 % likely at least 5 second recording
            srates(ii-1) = 1/(delsysraw.(fn5hzsec)(ii) - delsysraw.(fn5hzsec)(ii-1));
        end
        delsysForBeep.([namesfind{nn} 'srate']) =     sratesuse(nn);
        delsysForBeep.(namesfind{nn}) = delsysraw.(fn5hz);
        %{
        % plot interim figures 
        figure;plot(delsysraw.(fn5hzsec)(1:end-1), 1./diff(delsysraw.(fn5hzsec)))
        title(sprintf('%s %s',strrep( fn5hz ,'_',' '),'instantenous sampling rate')); 
        ylabel('sampling rate (Hz)'); 
        xlabel('seconds'); 
        set(gca,'FontSize',16); 
        
        % moving window sampling rate 
        figure;plot(delsysraw.(fn5hzsec)(1:end-1),...
            movmean( 1./diff(delsysraw.(fn5hzsec)) , 1e3 ) ) 
        title(sprintf('%s %s',strrep( fn5hz ,'_',' '),'moving window sampling rate (5e3 samples)')); 
        ylabel('sampling rate (Hz)'); 
        xlabel('seconds'); 
        set(gca,'FontSize',16); 
        %}
    end
    alligninfo = threshold_beep_finder(delsysForBeep,brraw);
    save(fullfile(ipadir,'ipad_allign_info.mat'),'alligninfo','delsysForBeep');
end
...
if isempty(ff)
    ...
    if exist(fullfile(ipadir,'ipad_event_indices_from_json.mat'),'file')
        ...
        delsysForEvent.srate = delsysForBeep.soundemgsrate;
        delsysForEvent.Erg1 = delsysForBeep.soundemg;
        eventsTable = transformJsonDatToEEGidx(timeDat, delsysForEvent);
        % get eeg idx of touch 
        delsysForTouch.srate = delsysForBeep.touchemgsrate;
        delsysForTouch.Erg1 = delsysForBeep.touchemg;
        [locuse] = peakFinder(delsysForTouch); 
        idxtimestamp = ceil(locuse .* delsysForTouch.srate);
        %% XXX 
        %% XXXX fix this - add an event centered on when left the trackpad 
        ...
        [leftTouchBRidx] = createEventMatrices(alligninfo,idxtimestamp,brraw,params);   % for now just using ipad beeps
        save(fullfile(ipadir,'ipad_event_indices_from_json.mat'),'eventsTable','leftTouchBRidx');
    end
end
...
if plottt0data
    ...
    plot_ipad_data_nicki_json(beepsInIdxs,brraw,794,ipadir,timeparams)
end
return ;
...
timeparams.extralines                  = 1; % plot extra line
timeparams.extralinesec                = 3000; % extra line location in seconds
timeparams.analysis                    = 'left_touch_sensor';
timeparams.filtertype                  = 'ifft-gaussian' ; % 'ifft-gaussian' or 'fir1'
addpath(genpath(fullfile(pwd,'from_nicki')));
plot_ipad_data_nicki_json(leftTouchBRidx,brraw,alligninfo.ecogsr,ipadir,timeparams)
