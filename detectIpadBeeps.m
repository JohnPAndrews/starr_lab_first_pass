function [outdat] = detectIpadBeeps(eegraw)
...
% on mouse up
    ...
    function FindBeeps(obj,event)
        ...
        ydat = eegraw.(rawfnms{pop.Value});
        thresh = hLine3.YData;
        [pks,locs,~,~] = findpeaks(ydat,eegraw.srate,...
        'MinPeakDistance',1,...
        'MinPeakHeight',thresh(1));
        set(ax,'nextplot','add');
        idxuse = locs > hLine1.XData(1) & locs < hLine2.XData(1);
        scatPlot = scatter(ax, locs(idxuse),pks(idxuse),300,'b');
        ttl1 = 'move sliders to select start, end and threshold';
        beeps = locs(idxuse);
        if isempty(beeps)
            ttl2 = sprintf('found 0 beeps');
