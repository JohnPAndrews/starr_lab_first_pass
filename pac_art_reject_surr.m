...
function [Comodulogram,pComodulogram,zComodulogram] = pac_art_reject_surr(data,srate,PhaseFreqVector,AmpFreqVector,bad_times,skip) 
%%uses Tort method to calculate PAC. Will do statistics if you specify a
%%variable "skip", will exclude noisy segments of data if you specify "bad
%%times"
%inputs:
...
PhaseFreqTransformed = zeros(length(PhaseFreqVector), data_length-length(bad_times));
    ...
    for ii=1:length(AmpFreqVector)
        ...
        evalc('AmpFreq=eegfilt_nicki(data,srate,Af1,Af2)'); % just filtering
...
%         PhaseFreq=eegfilt_nicki(data,srate,Pf1,Pf2); % this is just filtering
        evalc('PhaseFreq=eegfilt_nicki(data,srate,Pf1,Pf2)');
